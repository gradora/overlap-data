...
| `tracks/curated.json` | — ВЫГРУЗКА из приложения (шаг 5.1) | курируемые каталоги трасс: `geometry` (по каноничному слагу — высота, перепад, длина, старт→Т1, пит-лейн, повороты) и `facts` (по нормализованному имени трассы — круги, скорости, шины, рекорды, вехи). Выгружается тестом `CuratedTracksExportTests` (`TEST_RUNNER_CURATED_EXPORT=<путь>`), а не пишется руками: каждое число выстрадано аудитом, транскрипция гарантировала бы опечатку. Паритет с вкомпилированными таблицами держит тест |
