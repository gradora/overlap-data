...
//
// Вход — ТОЛЬКО уже лежащие на диске файлы: факты data/wec/facts (сняты тем
// же прогоном wec.ts), только что записанные index.json и standings.json.
// Сети здесь нет: прогон SEASON=<архив> собирает события из сохранённых
// фактов.
//
...
//     несёт, а лишние поля — это байты у каждой из ~2000 строк. ЗЕРКАЛА
//     БОЛЬШЕ НЕТ: добавить их = расширить extractFacts, поднять
//     WEC_FACTS_SCHEMA_VERSION и перекачать сезоны с fiawec (архив пока
