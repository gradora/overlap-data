...
/// Маркер продьюсера снапшота статики FOM (шаг 5.4). Та же роль, что у двух
/// выше. Пишется всегда, включая прогон «добирать нечего»: полный снимок — это
/// норма, а не простой, и вопрос к свежести здесь один — бежит ли продьюсер.
export const FOMSTATIC_MARKER = "f1/fom/_health.json";
...
export const PRODUCERS: ProducerSpec[] = [
  ...
  // Снапшот статики FOM: свой воркфлоу, суточный. Бюджет 3 суток — данные
  // архивные и не меняются, добирать после первой заливки нечего; сигнал нужен
  // только на «продьюсер отвалился». Отметку приносит маркер: шага в
  // snapshot.yml у него нет.
  {
    key: "fomstatic",
    script: "fomstatic",
    budgetDays: 3,
    workflow: ".github/workflows/fomstatic.yml",
    marker: FOMSTATIC_MARKER,
  },
  {
    key: "weclive",
    script: "weclive",
