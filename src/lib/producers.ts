...
export const PRODUCERS: ProducerSpec[] = [
  ...
  // Шаг СУТОЧНЫЙ: в часовых прогонах он штатно skipped, успех бывает раз в
  // сутки. Прежняя редакция ради дневной гранулярности health.json приводила
  // skipped к успеху (skippedIsSuccess) — и тем самым делала шаг НЕНАБЛЮДАЕМЫМ:
  // упавший суточный прогон через час затирался часовым «успехом», отметка
  // свежести обновлялась вечно, гейт не краснел никогда. Пока клиент умел
  // спросить у jolpica сам, цена была нулевой; после этапа 5.2 этот шаг —
  // ЕДИНСТВЕННЫЙ способ открыть в приложении следующий сезон, и его тихая
  // смерть означает, что сезон не откроется вовсе.
  // Аргумент, ради которого флаг стоял (лишний коммит health.json ломал
  // heartbeat, которым GitHub держит крон живым 60 дней), уходит вместе с
  // GitHub Actions: расписания переехали на Railway.
  { key: "nextseason", script: null, budgetDays: 2, workflow: SNAPSHOT },
  ...
  // Пострейс-канал питстопов из статики FOM (задача 3a) — второй ручной, по
  // ТОЙ ЖЕ причине класса источника, что fomstatic. Снимает стационарные
  // времена всех стопов гонки и спринта; заменил награды DHL в роли основного
  // канала. Запуск владельцем после этапа, дальше витрину пересобирают
  // бессетевые f1highlights/f1teams/f1beasts.
  //
  // Открытый вопрос: после переезда на Railway проверить egress оттуда. Если
  // 200 — продьюсер снимает manual, встаёт в snapshot.yml рядом с openf1, и
  // ручной ритуал исчезает целиком.
  {
    key: "f1pitstops",
    script: "f1pitstops",
    budgetDays: 3,
    workflow: "вручную: npm run f1pitstops",
    manual: "livetiming.formula1.com отдаёт 403 раннерам GitHub — снимать можно только локально",
  },

  // Награды DHL Fastest Pit Stop — третий ручной, по той же причине класса
  // источника (formula1.com блокирует раннеров GitHub) плюс осознанное
  // решение НЕ скрейпить FOM из крона. С задачи 3a это ПОСЛЕДНИЙ фолбэк
  // канала питстопов (f1/pitstops → openf1 → награды), живой ради сезонов до
  // 2025, где статика PitStopSeries отдаёт 403, а pit-факты openf1 мертвы.
  {
    key: "f1pitawards",
    script: "f1pitawards",
    budgetDays: 3,
