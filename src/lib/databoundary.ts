...
//
// 09–10.09.2026, этапы 1–4 заготовки openf1: `f1/openf1` ПЕРЕСТАЛ быть сырьём
// in-place — все 8 семейств конвертированы (keep-подмножество класса А,
// факт-конверты weather, R1 race_control), вербатим FIA удалён, каталог
// переведён в зону «заготовка». Правовой блокер сужен (полное сырьё остаётся
// в git-истории до фазы C — план §1, амендмент 8), но клиентская зависимость
// НЕ снята: clientReads: true — небывалое для заготовки сочетание, его держит
// отдельный поимённый инвариант в databoundary.test.ts.
...
export type Zone =
  ...
  /// витрина (приложение их как правило не читает; исключение — f1/openf1,
  /// где клиентский каскад архива 2023–24 жив до фазы 6). Промежуточное
  /// состояние сборки, которое можно держать в открытом репозитории без
  /// правового риска.
  | "заготовка"
...
/// Заготовки, которые читает приложение. Правовой предмет из них уже извлечён
/// (потому они не в блокерах сплита), но клиентская зависимость ЖИВА: экспорт
/// serve (фаза C) заготовки не публикует, и такой каскад там ослепнет. Список
/// существует, чтобы фаза 6 не потеряла этот хвост: снять clientReads можно
/// только вместе с уходом каскада на витрину.
export function clientReadStagings(): DataFamily[] {
  return DATA_FAMILIES.filter((f) => f.zone === "заготовка" && f.clientReads);
}
