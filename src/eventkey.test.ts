...
test("перенумерация раундов ключ не меняет", () => {
  // Ключ строится из слага и ключа митинга — раунда среди входов нет вовсе.
  const key = f1EventKey(2026, "albert-park", { kind: "meeting", meetingKey: 1279 });
  assert.equal(key, "f1-2026-albert-park-1279");
  assert.doesNotMatch(key, /-r?[17]$/, "раунд просочился в ключ");
});
