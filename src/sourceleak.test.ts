...
///
/// В скане только витрина и справочник — ОСОЗНАННО без заготовок, даже с
/// clientReads (f1/openf1 с 09.09.2026: заготовка, которую читает каскад
/// архива 2023–24). Сторож здесь — про текстовые маркеры в файлах, а сама
/// адресация f1/openf1 источник и так называет путём (уходит опаковыми id в
/// фазе D); форму содержимого держат walk-тест openf1facts (белые списки
/// ключей, шаблоны синтеза) и сторожа-throw писателя — чужой текст туда не
/// пролезает по построению. Скан 2000 файлов зеркала сверху ничего бы не
/// поймал, а исключение по имени каталога пришлось бы заводить всё равно.
