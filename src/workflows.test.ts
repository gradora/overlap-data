...
// serve:preview — инструмент разработчика (раскладка витрины для замера
// клиентских запросов, src/servepreview.ts): ничего не производит, кроны его
// звать не должны.
const NOT_PRODUCERS = new Set(["test", "typecheck", "serve:preview"]);
...
// Обобщение гейта выше на ВСЕ кроновые воркфлоу: continue-on-error глушит
// падение шага, и если его outcome никто ниже не читает, тревога гаснет молча
// при зелёных тестах — класс инцидента f1teams, только для канала письма.
// Появился вместе с f1live.yml: шаг openf1 получил continue-on-error
// (деградация зеркала не должна убивать бессетевые деривации), а письмо несёт
// отдельный гейт «Проверка зеркала OpenF1» — этот сторож не даст снести гейт
// «на время», оставив continue-on-error.
test("кроновые воркфлоу: outcome каждого continue-on-error шага кто-то читает", () => {
  for (const w of allWorkflows()) {
    if (!hasSchedule(w.text)) continue;
    const code = stripComments(w.text);
    // Порядок ключей в наших yml фиксирован (id → continue-on-error), поэтому
    // пары ловятся текстово, без YAML-парсера — как и весь этот файл.
    for (const m of code.matchAll(/id: (\w+)\s*\n\s+continue-on-error: true/g)) {
      const id = m[1];
      assert.ok(code.includes(`steps.${id}.outcome`),
        `${w.name}: шаг «${id}» под continue-on-error, но steps.${id}.outcome ` +
        `никто не читает — его падение не доедет до письма`);
    }
  }
});
...
/// Климатология прогноза — 5 archive-запросов на дальнее событие, ей не место
/// в ежечасном кроне: шаг forecast бежит каждый час, а typical-файлы
/// пересобирает только суточный слот через env-флаг. Пропади флаг из шага —
/// они молча перестали бы обновляться: «deferred» — штатный исход, который не
/// красит ничего. Сторож — по образцу шага «Сезон N+1».
test("сторож крона: шаг прогноза включает климатологию только суточным слотом", () => {
  const code = stripComments(readFileSync(join(WORKFLOWS_DIR, "snapshot.yml"), "utf8"));
  const step = code.split(/^\s+id: forecast$/m)[1] ?? "";
  assert.notEqual(step, "", "шаг forecast пропал из snapshot.yml");
  const block = step.split(/^\s+- name: /m)[0];
  assert.match(block, /FORECAST_TYPICAL/, "у шага forecast нет env-флага климатологии");
  assert.match(block, /37 3 \* \* \*/,
    "флаг климатологии не привязан к суточному расписанию — typical пересобирался бы ежечасно или никогда");
});
